...
# Remove WebDAV module/handler that block PUT/DELETE
...
if (-not ($handlers.SelectNodes("*[local-name()='remove']") | Where-Object { $_.GetAttribute("name") -eq "WebDAV" })) {
  ...
  # insert before aspNetCore add if possible
...
$existing = $envVars.SelectNodes("*[local-name()='environmentVariable']") | Where-Object { $_.GetAttribute("name") -eq "ASPNETCORE_ENVIRONMENT" }
if ($existing) {
  ...
  $ev.SetAttribute("value", "Production")
